...
export default function DictionaryKanji({ results }: IDictionaryKanji) {
  return (
    ...
    >
      {results ? (
        <>
          <Text fontSize={"20px"}>Significado</Text>
          <Text fontSize={"18px"}>{(results as any)?.kanji?.meaning?.english}</Text>
          <Divider />
          <KanjiReading
            reading={(results as any)?.kanji?.onyomi?.katakana}
            romaji={(results as any)?.kanji?.onyomi.romaji}
            type={"Onyomi"}
          />
          <Divider />
          <KanjiReading
            reading={(results as any)?.kanji?.kunyomi?.hiragana}
            romaji={(results as any)?.kanji?.kunyomi?.romaji}
            type={"Kunyomi"}
          />
          <Divider />
          <Text fontSize={"20px"}>Exemplos</Text>
